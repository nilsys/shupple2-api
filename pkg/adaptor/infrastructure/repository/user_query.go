...
func (r *UserQueryRepositoryImpl) IsExistMainMatchingUserMatchingHistory(id, matchingUserID int) (bool, error) {
	var row entity.UserMatchingHistory
	err := r.DB.Where("user_id = ? AND matching_user_id = ? AND user_main_matching_approve = true AND matching_user_main_matching_approve = true", id, matchingUserID).First(&row).Error
