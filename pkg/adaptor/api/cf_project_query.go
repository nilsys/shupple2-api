...
func (c *CfProjectQueryController) List(ctx echo.Context, ouser entity.OptionalUser) error {
	...
	projects, idIsFollowMap, err := c.CfProjectQueryScenario.List(c.Converters.ConvertCfProjectListInputToQuery(i), &ouser)
	if err != nil {
		return errors.Wrap(err, "failed list cf_project")
	}

	return ctx.JSON(http.StatusOK, c.ConvertCfProjectDetailListToOutput(projects, idIsFollowMap))
}
...
func (c *CfProjectQueryController) Show(ctx echo.Context, ouser entity.OptionalUser) error {
	...
	project, idIsFollowMap, err := c.CfProjectQueryScenario.Show(i.ID, &ouser)
	if err != nil {
		return errors.Wrap(err, "failed show cf_project")
	}

	return ctx.JSON(http.StatusOK, c.ConvertCfProjectDetailToOutput(project, idIsFollowMap))
...
func (c *CfProjectQueryController) ListSupported(ctx echo.Context, user entity.User) error {
	...
	projects, idIsFollowMap, err := c.CfProjectQueryScenario.ListSupported(&user, c.Converters.ConvertSupportedCfProjectListInputToQuery(i))
	if err != nil {
		return errors.Wrap(err, "failed list cf_project")
	}
